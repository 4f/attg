(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["main"], {
        ...
        /***/ (function (module, __webpack_exports__, __webpack_require__) {
            ...
            var HomeComponent = /** @class */ (function () {
                function HomeComponent(API, filter) {
                    this.API = API;
                    this.filter = filter;
                    this.filtered_data = [];
                    this.options = {
                ...
                HomeComponent.prototype.setCategories = function () {
                    var _this = this;
                    // this.CS.getCategories().subscribe(cats => this.categories = cats)
                    this.API.categories().subscribe(function (data) {
                        console.log(data);
                        _this.categories = data;
                        _this.norm_categories = _this._normalizeById(_this.categories);
                        _this.filter.data.category_ids.forEach(function (id) { return _this.norm_categories[id].__selected = true; });
                    });
                };
                HomeComponent.prototype.setCities = function () {
                    var _this = this;
                    this.API.cities().subscribe(function (data) {
                        _this.cities = data;
                        _this.norm_cities = _this._normalizeById(_this.cities);
                    });
                };
                HomeComponent.prototype.setData = function () {
                    var _this = this;
                    this.API.data().subscribe(function (data) {
                        console.log(data);
                        _this.data = data;
                        _this.filtered_data = _this.data;
                        _this.calculateSizeCaterories();
                        _this.doFilter();
                    });
                };
                HomeComponent.prototype.submit = function (e) {
                    this.doFilter();
                    e.preventDefault();
                    return false;
                };
                HomeComponent.prototype.getCategoryById = function (id) {
                    return this.categories.find(function (category) { return id === category.id; });
                };
                HomeComponent.prototype.getCityById = function (id) {
                    return this.cities.find(function (city) { return id === city.id; });
                };
                HomeComponent.prototype.calculateSizeCaterories = function () {
                    var _this = this;
                    this.categories.forEach(function (category) { return category.__size = 0; });
                    this.data.forEach(function (item) {
                        var category = _this.norm_categories[item.category];
                        if (!category)
                            return;
                        category.__size = category.__size + 1;
                    });
                };
                HomeComponent.prototype.doFilter = function () {
                    this.filtered_data = this.filter.get(this.data);
                };
                HomeComponent.prototype.changeCategory = function (event_or_bool, id) {
                    var result = (typeof event == 'boolean') ? event_or_bool : event_or_bool.target.checked;
                    this.norm_categories[id].__selected = result;
                    if (result)
                        this.filter.data.category_ids.push(id);
                    else
                        this.filter.data.category_ids = this.filter.data.category_ids.filter(function (category_id) { return category_id != id; });
                };
                HomeComponent.prototype.changeCity = function (id) {
                    this.filter.data.city_id = +id;
                };
                HomeComponent.prototype._normalizeById = function (array) {
                    var result = {};
                    if (array)
                        array.forEach(function (item) { return result[item.id] = item; });
                    return result;
                };
        ...
        /***/ (function (module, __webpack_exports__, __webpack_require__) {
            ...
            var ApiService = /** @class */ (function () {
                ...
                ApiService.prototype.data = function () {
        ...
        /***/ (function (module, __webpack_exports__, __webpack_require__) {
            ...
            var FilterService = /** @class */ (function () {
                function FilterService(route, router) {
                    this.route = route;
                    this.router = router;
                    this.data = {
                        city_id: 0,
                ...
                FilterService.prototype.get = function (data) {
                    var _this = this;
                    console.log(this.data);
                    var filtered_data = [];
                    data.forEach(function (item) {
                        if (_this._isCity(item)
                            && _this._isCategories(item)
                            && _this._isPrice(item)) {
                ...
                FilterService.prototype._setQuery = function (params) {
                    this.router.navigate([], {
                        relativeTo: this.route,
                        queryParams: params,
                    });
                };
                FilterService.prototype.setFromQuery = function () {
                    this._setFromQuery('price_end');
                    this._setFromQuery('price_start');
                    this._setFromQuery('category_ids');
                    this._setFromQuery('city_id');
                };
                FilterService.prototype._setFromQuery = function (name) {
                    var result = this.route.snapshot.queryParams[name];
                    if (!result)
                        return;
                    if (Array.isArray(this.data[name]))
                        this.data[name] = Array.isArray(result) ? result.map(function (item) { return +item; }) : [+result];
                    else
                        this.data[name] = +result;
                };
                FilterService.prototype._isPrice = function (item) {
                    return this.data.price_start <= item.price && this.data.price_end >= item.price;
                };
                FilterService.prototype._isCity = function (item) {
                    return this.data.city_id ? this.data.city_id == item.city : true;
                };
                FilterService.prototype._isCategories = function (item) {
                    if (this.data.category_ids.length <= 0)
                        return true;
                    if (!this.data.category_ids.some(function (value) { return !!value; }))
                        return true;
                    return this.data.category_ids.some(function (id) { return id == item.category; });
                };
